...
router.get("/listado-citas", (req, res) => {
  conexion.query(
    "SELECT fecha_cita, pacientes.nombres, pacientes.apellidos, pacientes.telefono, medicos.especialidad, medicos.consultorio, medicos.nombres nombresMedico, medicos.apellidos apellidosMedico FROM cita_medica, pacientes, medicos WHERE cedula_medico=medicos.cedula AND cedula_paciente=pacientes.cedula;",
    (error, resultado) => {
      if (error) {
        console.log("Ocurrio un error en la ejecución", error);
        res.status(500).send("Error en la ejecución");
...
router.post("/agregar-paciente", (req, res) => {
  ...
  conexion.query(
    `INSERT INTO pacientes (cedula, nombres, apellidos, fecha_nacimiento, telefono) VALUES (${cedula}, '${nombres}', '${apellidos}', '${fecha_nacimiento}', '${telefono}')`,
    (error, resultado) => {
      if (error) {
        console.log(error);
...
router.post("/agregar-cita", (req, res) => {
  ...
  conexion.query(
    `INSERT INTO cita_medica (cedula_medico, cedula_paciente, fecha_cita) VALUES (${cedula_medico}, ${cedula_paciente}, '${fecha_cita}')`,
    (error, resultado) => {
      if (error) {
        console.log(error);
        res.status(500).send("Ocurrio un error en la consulta");
